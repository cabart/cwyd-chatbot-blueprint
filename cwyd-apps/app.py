...
UI_TITLE = os.environ.get("UI_TITLE") or "Generative AI Chatbot"
UI_LOGO = os.environ.get("UI_LOGO")
UI_CHAT_LOGO = os.environ.get("UI_CHAT_LOGO")
UI_CHAT_TITLE = os.environ.get("UI_CHAT_TITLE") or "How can I help you today?"
UI_CHAT_DESCRIPTION = (
    os.environ.get("UI_CHAT_DESCRIPTION")
    or "This chatbot will answer questions based on the documents provided to it."
)
...
async def refdwnld(filepath):
    try:
        ...
        blob_url_with_sas = f"https://{blob_service_client.account_name}.blob.core.windows.net/{AZURE_BLOB_CONTAINER_NAME}/{blob_client.blob_name}?{sas_token}"
        response = requests.get(blob_url_with_sas)

        if filepath.lower().endswith('.pdf'):
            return Response(response.content, mimetype='application/pdf')

        return send_file(BytesIO(response.content), attachment_filename=filepath)
...
USER_AGENT = "PublicCloudBlueprint/GenAiChatbot/1.0.0"
...
def prepare_model_args(request_body):
    ...
    model_args = {
        ...
        "stream": request_body.get("should_stream", SHOULD_STREAM),
...
async def complete_chat_request(request_body):
    if USE_PROMPTFLOW and PROMPTFLOW_ENDPOINT and PROMPTFLOW_API_KEY:
        ...
        logging.warning(f"OPENAI-RESPONSE Type: {type(response)}")
...
async def conversation_internal(request_body):
    try:
        if request_body.get("should_stream", SHOULD_STREAM):  # override streaming in request
