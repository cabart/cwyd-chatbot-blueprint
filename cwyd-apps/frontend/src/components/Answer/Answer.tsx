...
export const Answer = ({
    ...
    const createCitationFilepath = (citation: Citation, index: number, truncate: boolean = false) => {
        ...
        if (citation.filepath) {
            ...
            if (truncate && citation.filepath.length > filePathTruncationLimit) {
                if(citation.filepath.includes("#page=")){
                  let fields = citation.filepath.split("#page=");
                  let filepath = fields[0];
                  const citationLength = filepath.length;
                  citationFilename = `${filepath.substring(0, 20)}...${filepath.substring(citationLength - 20)} - Page ${fields[1]}`;
                }else{
    ...
    const UnhelpfulFeedbackContent = () => {
        return (<>
            ...
            <div onClick={() => setShowReportInappropriateFeedback(true)} style={{ color: "#DE3919", cursor: "pointer" }}>Report inappropriate content</div>
        </>);
    }
