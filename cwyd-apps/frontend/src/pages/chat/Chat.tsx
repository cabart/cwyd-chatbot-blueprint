...
import ChatLogo from "../../assets/robot_red.png";
...
import { PDFCitationViewer } from "../../components/PDFCitationViewer/PDFCitationViewer";
...
const Chat = () => {
    ...
    const [activeCitationBasedAnswer, setActiveCitationBasedAnswer] = useState<string>(); // answer that was supplied by the assistent based on the citation
    ...
    const clearChat = async () => {
        ...
        if (appStateContext?.state.currentChat?.id && appStateContext?.state.isCosmosDBAvailable.cosmosDB) {
            ...
            if (!response.ok) {
                ...
                setActiveCitationBasedAnswer(undefined);
                setMessages([])
            }
        }
    ...
    const onShowCitation = (citation: Citation, origin: string) => {
        setActiveCitation(citation);
        setIsCitationPanelOpen(true);
        setActiveCitationBasedAnswer(origin);
    };


    const onViewSource = async (citation: Citation) => {
        if (citation.filepath) {
            const blobUrl = `/getblob/${citation.filepath}`;
                ...
                <Stack horizontal className={styles.chatRoot}>
                    <div className={styles.chatContainer}>
                        {!messages || messages.length < 1 ? (
                            <Stack className={styles.chatEmptyState}>
                                <img
                                    src={ui?.chat_logo ? ui.chat_logo : ChatLogo}
                                        ...
                                        {answer.role === "user" ? (
                                            ...
                                            answer.role === "assistant" ? <div className={styles.chatMessageGpt}>
                                                <Answer
                                                    ...
                                                    onCitationClicked={(c) => onShowCitation(c, answer.content)}
                                                />
                            ...
                            <Stack>
                                {appStateContext?.state.isCosmosDBAvailable?.status !== CosmosDBStatus.NotConfigured && <CommandBarButton
                                    ...
                                    styles={{
                                        ...
                                        root: {
                                            color: '#FFFFFF',
                                            background: "radial-gradient(109.81% 107.82% at 100.1% 90.19%, #DE3919 33.63%, #E1563B 70.31%, #EA8572 100%)"
                                        },
                                        rootDisabled: {
                                            background: "#F0F0F0"
                                        }
                    ...
                    {/* Citation Panel */}
                    {messages && messages.length > 0 && isCitationPanelOpen && activeCitation && activeCitationBasedAnswer && (
                        <Stack.Item className={styles.citationPanel} tabIndex={0} role="tabpanel" aria-label="Citations Panel">
                            ...
                            <div tabIndex={0}>
                            {activeCitation.url && (activeCitation.url.endsWith(".pdf") || activeCitation.url.includes(".pdf#page=")) ? (
                                <div>
                                    <PDFCitationViewer pdfURL={`/getblob/${activeCitation.filepath}`} pageNumber={Number(activeCitation.url.split(".pdf#page=")[1])} searchText={activeCitationBasedAnswer} />
                                </div>
                                ) : (
                                <ReactMarkdown
                                    linkTarget="_blank"
