...
class PdfTextSplitter(TextSplitter):
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        final_final_chunks = [chunk for chunk, chunk_size in merge_chunks_serially(
            final_chunks, self._chunk_size, url_dict)]
...
class Document(object):
    ...
    Attributes:
        ...
        page_number (Optional[int]): The number of the page where the chunk is located in the document (pdf only)
    """
...
class HTMLParser(BaseParser):
    ...
    def parse(self, content: str, file_name: Optional[str] = None) -> Document:
        ...
        if "<h" in content or "/>" in content:
            return self.parseHtml(content, file_name)
        elif "# " in content:
            return self.parseMd(content, file_name)
        return Document(content, title=file_name)

    def parseMd(self, content: str, file_name: Optional[str] = None) -> Document:
        def first_line():
            lines = content.split("\n")
            if len(lines) > 0:
        ...
        title = None
        if len(parts) > 0:
            heading_and_tail = parts[0].split("\n")
            if len(heading_and_tail) > 0:
                title = heading_and_tail[0]
        if title is None:
            title = first_line()
        if title is None:
            title = file_name
        return Document(content, title=title)

    def parseHtml(self, content: str, file_name: Optional[str] = None) -> Document:
        """Parses the title out of the given html content.
        Args:
            content (str): The content to parse.
            file_name (str): The file name associated with the content.
        Returns:
...
def downloadBlobUrlToLocalFolder(blob_url, local_folder, blob_service_client):
...
def extract_content_form_recognizer(file_path, form_recognizer_client, use_layout=False):
    offset = 0
    page_map = []
    model = "prebuilt-layout" if use_layout else "prebuilt-read"
    #TODO Technical Debt: switch to new version of form recognizer (>=2023-10-31) to have better docx support
    ...
    page_texts = [text for _,_,text in page_map]
    return page_texts


def extract_pdf_content(page_texts: List):
    # Filter out pages:
    # Remove table of contents identified by regex.
    # Find all table row containing two columns:
    # - section numbering followed by text followed by at least 10 '.'
    # - page number
    regex_pattern = r"(<tr><td>\b\d+(?:\.\d+)*\b.?[ |\w]+\.{10,}</td><td>\d+</td></tr>)"
    # for table of contents pages we return None, which makes sure that we do not skip the page numbering
    # remove superfluous text that is often extracted from pdfs
    page_texts = [page_text.replace(":selected:", "").replace(":unselected:", "") if len(list(re.finditer(regex_pattern, page_text))) < 3 else None for page_text in page_texts]
    return page_texts
...
def get_embedding(text, embedding_model_endpoint=None, embedding_model_key=None, azure_credential=None):
    ...
    try:
        ...
        client = AzureOpenAI(api_version=api_version,
                             azure_endpoint=base_url, api_key=api_key)
...
def chunk_file(
    ...
    cracked_pdf = False
    if file_format in ["pdf"]:  # update to documentintelligence >= 2023-10-31
        if form_recognizer_client is None:
            raise UnsupportedFormatError(
                "form_recognizer_client is required for pdf files")
        page_texts = extract_content_form_recognizer(
            file_path, form_recognizer_client, use_layout=use_layout
        )

        import markdownify
        page_texts = [markdownify.markdownify(page, heading_style="ATX") for page in page_texts]

        cracked_pdf = True
        if file_format == "pdf":
            content = extract_pdf_content(page_texts)
        else:
        ...
        with open(file_path, "rb") as f:
            import mammoth
            import markdownify
            content = markdownify.markdownify(mammoth.convert_to_html(f).value, heading_style="ATX")
            cracked_pdf = True
    else:
        try:
    ...
    if type(content) == str:  # create chunks from full document content
    ...
    elif type(content) == list:  # create chunks from pages and add page number to index
        page_data = [chunk_content(
        ...
        print("creating pdf document pagewise ...")
        chunks = []
        total_files = 0
        skipped_chunks = 0
        ...
        for page in page_data:
            ...
            skipped_chunks += page.skipped_chunks
            num_unsupported_format_files += page.num_unsupported_format_files
            num_files_with_errors += page.num_files_with_errors
        return ChunkingResult(chunks=chunks, total_files=total_files, skipped_chunks=skipped_chunks, num_unsupported_format_files=num_unsupported_format_files, num_files_with_errors=num_files_with_errors)
...
def process_file(
    ...
    try:
        ...
        print(f"INFO-PROCESS-FILE: Processing {file_path}")
        result = chunk_file(
            file_path,
            ignore_errors=ignore_errors,
            num_tokens=num_tokens,
        ...
        for chunk_idx, chunk_doc in enumerate(result.chunks):
            if chunk_doc.page_number is not None:
                # pseudo url parameter to extract page_number from url in UI
                # add one to create a page number starting from 1 and not 0
                chunk_doc.filepath = rel_file_path + f"#page={chunk_doc.page_number+1}"
            else:
...
def chunk_blob_container(
    ...
    with tempfile.TemporaryDirectory() as local_data_folder:
        ...
        downloadBlobUrlToLocalFolder(
            blob_url, local_data_folder, blob_service_client)
        print(f'Downloaded.')
...
class SingletonFormRecognizerClient:
    ...
    def __setstate__(self, state):
        url, key = state
        self.instance = DocumentAnalysisClient(endpoint=url, credential=AzureKeyCredential(
